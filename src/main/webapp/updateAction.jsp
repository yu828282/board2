...
<body> 
	...
	String[] checkedFiles = multipartRequest.getParameterValues("checkedValue");	
	
	if(checkedFiles != null){
		for(String checkedFile : checkedFiles){
			System.out.println(checkedFile);
			int result = fileDAO.delete(Integer.parseInt(checkedFile));
			if(result == -1){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('파일 삭제에 오류가 발생했습니다. 관리자에게 문의해주세요.')");
 		...
 		fileRealName = multipartRequest.getFilesystemName(nextExist);	  	//리네임
 	 	if(fileName != null){
 	 		...
 	 		if(file.exists()){  			//파일이 존재하는가
 	 			...
 	 			resultFile = fileDAO.write(bbsId, fileName.replaceAll("\\s+","%20"), fileRealName.replaceAll("\\s+","%20"), fileSize);
 	 		}
 		}
 	  }
	//파일첨부 부분
